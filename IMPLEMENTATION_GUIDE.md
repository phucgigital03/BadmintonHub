...
| **Week 4** | event-service + integration | event-service + smoke test toàn hệ thống (frontend đã gộp vào Day 4) |

> ▶️ **Ưu tiên hiện tại (ngoài lịch 4 tuần gốc)**: `ai-service` THẬT — đối soát chứng từ thanh toán hỗ trợ AI
...
> cuối Week 2) — spec `UC_Chatting.md`, build theo 6 phase paste-ready.
...
Week 2: Day 8 ‖ Day 9 (payment ‖ escrow — DB khác nhau, độc lập)
         ...
         Day 10.5 (▶️ ƯU TIÊN) ai-service đối soát chứng từ — payment-service (PHẦN A) → ai-service Python (PHẦN B)

Week 3: Day 11 → Day 12 (matchmaking creation → player join, sequential)
         Day 13 ‖ Day 14 (notification ‖ coach — hoàn toàn độc lập)

Week 4: Day 20 (event-service + smoke test) — frontend đã gộp vào Day 4
         (Day 16–19 để trống)
```
> ⚠️ **Day 10.5 lệch stack**: `ai-service` nay là **Python** (FastAPI/LangGraph, đăng ký Eureka qua `py-eureka-client`,
> gỡ khỏi `<modules>` Maven) và **phụ thuộc payment-service** (bảng `bank_transactions` ở PHẦN A) → làm PHẦN A trước PHẦN B.
...
> Day 4 thực tế chạy **3 phase TUẦN TỰ** (các layer phụ thuộc nhau nên không song song được) — commit giữa mỗi phase:

```
1.  /plan-feat user-service auth core
    → duyệt plan.

2.  "Dùng agent spring-builder Phase 1: pom deps + enum AuthProvider/RoleName + entity
     User/Role/AuditLog (roles qua @ManyToMany join user_roles, KHÔNG entity UserRole riêng;
     schema ddl-auto, KHÔNG Flyway) + repository. Chưa làm service/controller. Xong COMMIT."  (95b93fd)

3.  "Dùng spring-builder Phase 2: DTO + JwtTokenProvider (issuer HS256) + EmailService(Impl) +
     AuthService(Impl @Service(\"authService\")) + SecurityConfig beans. Xong COMMIT."  (7e7f272)

4.  "Dùng spring-builder Phase 3: JwtAuthFilter + SecurityConfig filter chain + AuthController.
     Xong COMMIT."  (a890e8e)

5.  ⏳ HOÃN (chưa chạy): /write-tests user-service auth → /code-review → /security-review → /done-check user-service
```

> Đây là ví dụ **tuần tự** (B phụ thuộc A). Muốn ví dụ **song song**, xem Day 11/13 — 2 subagent độc lập chạy cùng lúc.
...
### Day 4: user-service — Auth core ✅ (backend HOÀN THÀNH · frontend bên dưới)

**Mục tiêu**: Register → Email verify → Login → JWT token hoạt động.

Tasks:
- [x] Entity: `User`, `Role`, `AuditLog` + enum `AuthProvider`/`RoleName` (roles qua **`@ManyToMany` join `user_roles`** — KHÔNG có entity `UserRole` riêng; within-service FK OK)
- [x] register / verify-email / login / refresh / logout
- [x] `JwtAuthFilter` per-service: **re-validate** Bearer token bằng `common-security` `JwtUtil` (defense-in-depth)
- [ ] **Test** (unit + integration) — HOÃN, chưa làm: `/write-tests user-service auth`
- [ ] **Frontend** (toàn bộ app) — xem mục "Frontend — toàn bộ app" cuối Day 4

**✅ Đã build (current state — 3 commit `95b93fd` / `7e7f272` / `a890e8e`, `mvn -pl user-service -am clean install -DskipTests` SUCCESS)**:

- **Schema** = Hibernate **`ddl-auto:update`** (KHÔNG Flyway — chưa service nào dùng Flyway). `@EnableJpaAuditing` bật.
- **ENTITY** (extend `BaseAuditEntity`, PK UUID `@GeneratedValue(UUID)`):
  - `User`: email (unique), passwordHash (nullable cho OAuth), fullName, phone, isEmailVerified (default false),
    authProvider (enum LOCAL/GOOGLE), googleId (unique nullable), refreshTokenHash, fcmToken,
    deletedAt (soft delete `@Where("deleted_at IS NULL")`), roles `@ManyToMany(EAGER)` qua join `user_roles`.
  - `Role` (name = enum `RoleName` ADMIN/STAFF/COACH/USER, unique). `AuditLog` (tạo sẵn, chưa wire).
- **SERVICE LAYER** = interface + impl (convention toàn dự án — `java-spring.md`):
  - `service/AuthService` (interface, giữ nested record `LoginResult`/`RefreshResult`) + `service/impl/AuthServiceImpl`
    annotated **`@Service("authService")`** (giữ tên bean cho SpEL `@authService.isEmailVerified`).
  - `service/EmailService` (interface) + `service/impl/EmailServiceImpl`.
- **TOKEN**: `security/JwtTokenProvider` = **issuer** HS256 (`common-security` `JwtUtil` CHỈ validate).
  ⚠️ Claims khớp hợp đồng `JwtUtil`: `sub`=userId(UUID), `roles`=`List<String>` (claim `JwtUtil.CLAIM_ROLES`,
  vd `["ROLE_USER"]`), `jti`=UUID — sai thì gateway đọc lệch.
- **ENDPOINT** (base `/api/auth`, error shape `{code, message, timestamp}`):
  - `POST /register`: `@Valid`, BCrypt(12) hash, gán ROLE_USER, sinh verify token → Redis `email:verify:{token}` TTL 24h,
    gửi email. Rate limit Redis `rate_limit:register:{ip}` TTL 3600s max 5 (fail-open nếu Redis chết).
  - `GET /verify-email?token=`: đọc Redis token (single-use, delete sau đọc) → set isEmailVerified=true.
  - `POST /login`: verify credential → access token JWT HS256 TTL 15m. **Refresh token = opaque `userId:UUID`**
    (KHÔNG phải JWT), lưu BCrypt hash vào `users.refresh_token_hash`, trả cookie **HttpOnly SameSite=Strict** path `/api/auth`.
  - `POST /refresh`: đọc refresh cookie, so BCrypt hash → cấp access token mới + **rotate** refresh token.
  - `POST /logout`: blacklist `jti` vào Redis `session:blacklist:{jti}` TTL = thời gian còn lại; idempotent + clear cookie.
- **EMAIL**: `EmailServiceImpl` — có `SENDGRID_API_KEY` → gửi thật; trống → **log verify link** ra console (dev chạy được không cần SendGrid). Dep `com.sendgrid:sendgrid-java:4.10.2` pin trong user-service pom.
- **SECURITY** (defense-in-depth): `JwtAuthFilter` (`OncePerRequest`) re-validate Bearer bằng `JwtUtil` → dựng Authentication TỪ CLAIMS, **KHÔNG** trust header `X-User-Id/X-User-Roles`. `SecurityFilterChain`: csrf disable, STATELESS, `/api/auth/**` + `/actuator/health|info` permitAll, `@EnableMethodSecurity`.
- **pom deps thêm**: `common-security`, `spring-boot-starter-validation`, `common-test` (scope test), `sendgrid-java`.

**Định nghĩa Done** (backend — verified bằng curl, test tự động còn HOÃN):
```bash
curl -X POST http://localhost:3000/api/auth/register \
  -H 'Content-Type: application/json' \
  -d '{"email":"test@test.com","password":"123456","fullName":"Test User"}'   # → 201
# verify email bằng token (lấy từ log console khi chưa có SendGrid) → login → nhận JWT → gọi endpoint authenticated qua Gateway OK
# ⏳ Còn lại: /write-tests user-service auth (unit + integration *IT) + /code-review + /security-review + /done-check
```

**🚀 Pro Workflow đã dùng (current state)**

Service phức tạp nhất Week 1 → theo **Công thức Agent Team**. Đã thực thi:

1. **Plan** — `/plan-feat user-service auth core` → duyệt (chốt: ddl-auto giữ nguyên · Role+role join · email log link khi thiếu key · build = Agent Team).
2. **3 phase `spring-builder` TUẦN TỰ, commit giữa mỗi phase** (dependency-aware — layer phụ thuộc nhau nên KHÔNG song song):
   - **Phase 1** (`95b93fd`): pom deps + enums + entities (`User`/`Role`/`AuditLog`) + repositories.
   - **Phase 2** (`7e7f272`): DTOs + `JwtTokenProvider` + `EmailService(Impl)` + `AuthService(Impl)` + `SecurityConfig` beans (`BCryptPasswordEncoder(12)` + `JwtUtil`).
   - **Phase 3** (`a890e8e`): `JwtAuthFilter` + `SecurityConfig` filter chain + `AuthController`.
3. **Chốt** — ⏳ HOÃN theo yêu cầu (chưa chạy): `/write-tests user-service auth` · `/code-review` · `/security-review` (JWT secret env · BCrypt ≥10 · blacklist TTL = token remaining · KHÔNG log password) · `/done-check user-service`.

**🖥️ Frontend — toàn bộ app** (build tối nay, mục con của Day 4)

> Gom toàn bộ frontend (trước đây tách Day 16–19 + EventsPage Day 20) vào đây. **Backend hiện chỉ có auth (Day 4)** → page gọi API chưa tồn tại phải handle `isLoading`/`isError` của React Query, **KHÔNG crash**. Build UI theo contract trong `frontend.md` để page tự "sáng" khi backend các Day sau xong.
>
> **Fallback mock data**: khi query lỗi (API chưa có), page **hiển thị 2–3 bản ghi mock** (đúng shape response trong `frontend.md` / ERD) để UI thấy được layout + flow, kèm badge/banner nhỏ "Dữ liệu mẫu — API chưa sẵn sàng" + `toast.error(err.response?.data?.message ?? 'Lỗi hệ thống')`. Để mock gọn 1 chỗ: `src/api/mockData.ts` (export `mockCourts`, `mockMatches`, `mockCoaches`, `mockEvents`… mỗi cái 2–3 item) và **chỉ dùng trong nhánh `isError`** (vd `const data = query.isError ? mockCourts : query.data`). Mock chỉ là placeholder dev — **mutation (POST/PUT) KHÔNG mock**, vẫn để báo lỗi thật; gỡ/để nguyên khi backend thật xong (data thật override).

```
Đọc trước: .claude/rules/frontend.md (File Structure, axiosClient, authStore, slotColors,
PaymentScreenProps, useMatchSocket). Env đã có: frontend/.env (VITE_API_URL=:3000 gateway, VITE_WS_URL=:3004).

SCAFFOLD: Vite + React 18 + TypeScript + Tailwind theo File Structure trong frontend.md.
QueryClientProvider bọc app · react-hot-toast · i18n react-i18next (vi.json/en.json, mọi string qua t()) ·
React Router v6 createBrowserRouter (public + protected routes).

CORE PLUMBING:
- src/api/axiosClient.ts: baseURL=VITE_API_URL; request interceptor gắn Bearer accessToken từ authStore;
  response interceptor silent refresh khi 401 (POST /api/auth/refresh, cookie-based, _retry flag). MỌI HTTP qua client này.
- src/store/authStore.ts (Zustand): { accessToken, user, setAuth, clearAuth }. KHÔNG để server data trong Zustand.
- src/api/mockData.ts: mockCourts/mockMatches/mockCoaches/mockEvents… (mỗi cái 2–3 item, đúng shape response).
  Page CHỈ dùng mock trong nhánh isError (data = query.isError ? mock : query.data) + banner "Dữ liệu mẫu".
- ProtectedRoute + RoleGuard components.

AUTH PAGES (chạy được NGAY với backend Day 4): LoginPage, RegisterPage, VerifyEmailPage — React Hook Form + Zod.
  register → verify email → login → redirect dashboard; silent refresh 401 hoạt động.

COURTS + BOOKING (backend chưa có → mock 2–3 item khi isError): CourtsPage (search district/type/date qua useQuery +
  react-leaflet map), CourtDetailPage + SlotGrid (màu theo slotColors trong frontend.md), click slot AVAILABLE →
  booking modal → useMutation POST /api/bookings. PaymentScreen component theo PaymentScreenProps (bank info + QR +
  countdown + upload zone; multipart POST /api/payments/{id}/proof; disable confirm tới khi chọn ảnh).

MATCHMAKING + REAL-TIME (backend chưa có → mock 2–3 item khi isError): MatchesPage (filter date/skill), MatchDetailPage (slot counter + join),
  src/hooks/useMatchSocket.ts (io(VITE_WS_URL), emit 'join-match-room', listen 'slot-updated' → setQueryData;
  socket CHỈ qua hook này), create match modal (Zod: totalSlots chẵn 2..16, pricePerPerson≥0, date tương lai) →
  PaymentScreen (MATCH_HOST), join → PaymentScreen (price_per_person). notificationStore + NotificationBell.

ADMIN + COACH (backend chưa có → mock 2–3 item khi isError): AdminPage (RoleGuard ['STAFF','ADMIN'] — tab Proof Review confirm/reject,
  tab Matches force cancel, tab Bookings, tab Refunds form manual refund POST /api/payments/{id}/refund),
  CoachesPage (search specialty/rate/rating), CoachDetailPage (enroll → PaymentScreen COACH_ENROLLMENT),
  DashboardPage (my bookings/matches/enrollments/notifications).

EVENTS (backend chưa có → mock 2–3 item khi isError): EventsPage list events → buy ticket → PaymentScreen (EVENT_TICKET).
```

**Định nghĩa Done (frontend)**: `cd frontend && npm install && npm run dev` boot OK; register/login chạy trên browser với gateway :3000, silent refresh 401 hoạt động; các page còn lại (API chưa có) **hiện 2–3 mock item + banner "Dữ liệu mẫu"** thay vì trắng/crash.
...
> ⏳ **CHƯA LÀM** (trạng thái 2026-06): booking↔payment đã build + đã audit money-safety (5 vòng) →
> Day 10 chỉ còn (a) `club_reviews` (CHƯA có entity/endpoint), (b) integration test cột mốc nối cả luồng đặt sân.
> ▶️ **User ưu tiên làm AI feature (mục `Day 10.5` ngay dưới) TRƯỚC Day 10.**
...
Viết test mô phỏng end-to-end luồng đặt sân (header + items):
  ...
  6. (TÙY CHỌN) escrow ghi transaction đúng — escrow đang NGỦ tới Day 11 (chưa có producer match.*),
     và handler payment.player.confirmed còn thiếu guard matchId==null → có thể bỏ bước này cho cột mốc Week 2.
     Assert tới "Booking CONFIRMED + mọi slot RESERVED" là đủ.
...
### Day 10.5 — ai-service: Đối soát chứng từ thanh toán hỗ trợ AI (Python · ▶️ ƯU TIÊN LÀM TRƯỚC)
...
biên lai, tra giao dịch ngân hàng theo `order_code`, chấm điểm **tất định** → trả ảnh-tag (field_locations) +
scorecard + match level gợi ý (HIGH/MEDIUM/LOW/SUSPECT/ERROR). **Công cụ HỖ TRỢ — STAFF vẫn là người bấm
Xác nhận / Từ chối.**
...
- **Nguồn ground-truth ngân hàng** = **import sao kê VCB thủ công** (bước đầu): STAFF upload file sao kê VCB
  ...
  — chừa **extension point** để sau nâng SePay webhook ghi vào **cùng bảng** mà KHÔNG sửa tool/graph/policy.

**Tiền đề đã xác minh (không cần làm lại)**:
- `payments.order_code` (Long bigserial, hiển thị `#184`) **đã có + đã expose** trong `PaymentResponse` + response
  của `initiate` → **tái dùng**, KHÔNG sinh mã mới.
- Ảnh proof lấy bytes qua `GET /api/payments/{id}/proofs` → `imageUrl` (Cloudinary URL / `local-fallback://`).
- Frontend đã có `ProofDetailModal` (xem ảnh + confirm/reject) trong `frontend/src/pages/admin/AdminPage.tsx`
  → feature cắm vào đây.
...
- Entity/bảng bank_transactions (payment_db): id UUID, bank_ref (mã GD — UNIQUE, dedupe key), amount decimal,
  transferred_at, memo/description (chứa order_code), sender_name, account_number, raw_line,
  source enum {MANUAL_IMPORT, SEPAY}, audit. Index UNIQUE(bank_ref) + index lookup theo memo/amount.
- Ingestion PLUGGABLE: interface BankStatementParser + impl VcbCsvStatementParser (VCB CSV/Excel).
  POST /api/bank-transactions/import (multipart, @PreAuthorize hasAnyRole('STAFF','ADMIN')):
  parse -> dedupe theo bank_ref (skip trùng) -> bulk insert -> trả { inserted, skipped }.
- Lookup cho AI tool: GET /api/bank-transactions/lookup?orderCode=&amount= (internal, STAFF/service token):
  match memo chứa order_code + amount khớp (tolerant) -> trả BankTransaction (hoặc rỗng).
...
- Stack: FastAPI + Uvicorn · LangGraph (graph + human-in-the-loop interrupt + checkpoint) · Pydantic v2
  ...
  structlog · OpenTelemetry -> Zipkin · py-eureka-client (giữ lb://ai-service).
- Pydantic models: MatchLevel(HIGH/MEDIUM/LOW/SUSPECT/ERROR), VerificationResult, ExtractedProof
  (amount/orderCode/senderName/bankName/imageQuality/field_locations/criteria), CriterionCheck,
...
- Endpoint: POST /api/ai/payment-proofs/{paymentId}/verify[?refresh=true] -> VerificationResult
  (read-only, cache Redis; refresh=true mới chạy lại). Approve/reject DÙNG cơ chế STAFF hiện có — KHÔNG path nào tự approve.
- Audit: snapshot mỗi run vào verification_log (scorecard + field_locations + tool result + match level
  + model + prompt version). Secrets qua env.

PHẦN C — frontend (mở rộng ProofDetailModal có sẵn trong AdminPage.tsx):
- Mở modal "Xem" -> gọi verify qua React Query (loading, cache). Render: badge match level (màu) +
  ảnh gốc + overlay tag từ field_locations + scorecard (sub-score + lý do) + dòng giao dịch ngân hàng khớp +
  nút "Chạy lại đối soát" (refresh=true).
- Nút Xác nhận / Từ chối GIỮ NGUYÊN, chỉ STAFF bấm. (Tùy chọn: nút STAFF upload sao kê VCB -> /api/bank-transactions/import.)

Test + eval (xanh trước khi tin): unit mọi nhánh policy (HIGH/MEDIUM/LOW/SUSPECT/ERROR) · "agent KHÔNG ra HIGH
khi G=false" · verify read-only (không đổi trạng thái ở mọi nhánh) · cache (refresh mới chạy lại) ·
structured-output lỗi -> fallback · pHash trùng -> SUSPECT · tool timeout -> ERROR/thủ công · eval harness ảnh có nhãn.
```

**Định nghĩa Done** (theo tiêu chí nghiệm thu UC): bấm "Xem" mới chạy (không chạy lúc upload) · đúng **1 agent LLM**
(không vision-agent riêng, không model forgery) · `verify` không đổi trạng thái · **HIGH chỉ khi `G=true`** (có test
chứng minh agent không vượt cổng cứng) · cả 5 nhánh (HIGH/MEDIUM/LOW/SUSPECT/ERROR) đều về `human_review` · pHash
trùng → SUSPECT, tool lỗi/timeout → ERROR/thủ công · mỗi run có snapshot audit + model/prompt version · import sao kê
VCB dedupe đúng theo `bank_ref` · secrets không trong source · test + eval xanh.
...
**Mục tiêu**: Kênh chat hỗ trợ real-time giữa **Khách** (USER/COACH) và **Nhân viên** (STAFF). Khách mở 1 luồng →
vào **hàng đợi chưa-gán chung**; STAFF **claim** về **inbox riêng** → chỉ STAFF đó (+ ADMIN giám sát) xử lý. Tin
đẩy tức thì qua **STOMP over WebSocket**, trạng thái **`sent → delivered → read`** + typing, **gửi ảnh**
(Cloudinary), **thông báo offline** qua Kafka → notification-service (email/FCM). Lưu MongoDB `chat_db`.

**Quyết định đã chốt (bake vào guide — KHÔNG hỏi lại khi implement)**:
- **Transport = STOMP over WebSocket** (`spring-boot-starter-websocket`), FE `@stomp/stompjs` — **KHÔNG socket.io**
  (socket.io giữ cho matchmaking). Per-user queue `convertAndSendToUser` + `setUserDestinationPrefix("/user")`.
- **Mô hình = per-staff private inbox** + hàng đợi chưa-gán chung (OPEN→claim→ASSIGNED→CLOSED, reopen giữ
  `assignedStaffId`, ADMIN xem tất cả). **3-state delivery** suy từ timestamp (`createdAt`/`deliveredAt`/`readAt`).
- **Lưu trữ = MongoDB `chat_db`** — **container RIÊNG `mongodb-chat:27018`** (DB-per-service vật lý, đã pre-provision trong docker-compose; tách hẳn notification-service) · **single-document Outbox**
  (`notifyStatus` trong message doc — Mongo standalone không multi-doc txn) · index/keyset/computed theo **§F**.
- **Bảo mật/vận hành = bake §G**: auth ở **STOMP CONNECT frame** (gateway `/ws/**` public) · **authz per-frame**
  ...
  `activeCustomerId`** (1 thread mở/khách) · presence **ref-count** (offline khi tập session rỗng).
...
- `common-security` **`JwtUtil`** (`com.badmintonhub.security`: `parseAndValidate`/`getUserId`/`getRoles`/`getJti`)
  → dùng cho `JwtAuthFilter` (REST) **và** `StompAuthChannelInterceptor` (CONNECT frame). Token = nguồn danh tính.
- **notification-service** = template Mongo (`spring-boot-starter-data-mongodb` + exclude DataSource) + Kafka
  consumer; **booking-service `KafkaConsumerConfig`** = template manual-ack + `DefaultErrorHandler` + DLT 2/4/8s.
- **payment-service `CloudinaryService`/`CloudinaryServiceImpl`/`CloudinaryProdGuard`** = mẫu upload ảnh (degrade
  `local-fallback://` dev · chặn boot prod). **Container `mongodb-chat` (port 27018) đã có sẵn trong docker-compose** — KHÔNG dùng chung server với notification-service.

> ⚠️ **Phụ thuộc**: PHASE 4 (offline-notify) cần **notification-service (Day 13)**. Nếu chưa build: **vẫn làm
> producer side** (chat outbox → Kafka) ngay; **consumer side để lại** tới Day 13 — tin KHÔNG mất, chỉ chậm
> email/FCM. PHASE 1–3 (chat real-time) + PHASE 5 (FE) chạy độc lập không cần notification-service.

---

**PHASE 0 — Nạp ngữ cảnh (làm 1 lần đầu session)**
```
Đọc trước (đọc kỹ, KHÔNG bỏ qua): UC_Chatting.md (SPEC ĐẦY ĐỦ — 5 UC + Phụ lục A→G),
.claude/rules/architecture.md, .claude/rules/eureka-config.md, .claude/rules/rbac-security.md,
.claude/rules/kafka-patterns.md, .claude/rules/redis-patterns.md, .claude/rules/database.md,
.claude/rules/java-spring.md, .claude/rules/testing.md, .claude/rules/frontend.md.

Tôi sẽ build chat-service theo 6 phase (PHASE 1→6 ở dưới), backend-first. TRƯỚC MỖI PHASE: chạy plan mode,
...
**PHASE 1 — Scaffold chat-service (port 3011) + gateway + Eureka** — *tiền đề §0.2/§0.3*
```
Chạy /new-service chat-service (port 3011) rồi điều chỉnh theo chat:
- Stack: Spring Web + spring-boot-starter-websocket + spring-boot-starter-data-mongodb +
  spring-boot-starter-security + common-security + common + common-test(test) + eureka-client +
  spring-kafka + spring-boot-starter-data-redis. KHÔNG JPA → application.yml exclude
...
- Root pom.xml: thêm <module>chat-service</module>.
- api-gateway/application.yml: thêm 2 route → (1) id=chat-service uri=lb://chat-service Path=/api/chat/**
  (2) id=chat-ws uri=lb://chat-service Path=/ws/** . Và THÊM /ws/** vào PUBLIC_PATHS của JwtAuthenticationFilter
  (gateway KHÔNG verify được WS handshake — auth dồn về STOMP CONNECT frame, §G.1).
- Cập nhật bảng port: thêm "chat-service | 3011" vào .claude/rules/architecture.md + .claude/rules/eureka-config.md.
Verify: mvn -pl chat-service spring-boot:run → đăng ký Eureka UP, GET /actuator/health = 200. Tắt service sau verify.
```
...
- @Document conversations + messages đúng §E.4 (enum status OPEN/ASSIGNED/CLOSED · type TEXT/IMAGE ·
  notifyStatus NONE/PENDING/SENT · notifyTarget · snapshot customerName/senderName · counters customerUnread/
  staffUnread · activeCustomerId · archivedAt). Index theo §F.2 qua @CompoundIndex / MongoIndexInitializer:
  messages {conversationId:1,_id:-1} · UNIQUE {conversationId:1,senderId:1,clientMsgId:1} · partial {notifyStatus:1}
...
- WebSocketConfig (@EnableWebSocketMessageBroker): registerStompEndpoints("/ws").setAllowedOrigins(${FRONTEND_URL})
  ; enableSimpleBroker("/topic","/user") · setApplicationDestinationPrefixes("/app") · setUserDestinationPrefix("/user")
  ; giới hạn message-size (setMessageSizeLimit/setSendBufferSizeLimit — chống frame khổng lồ, §G.1).
...
- Presence ref-count (§G.6): @EventListener SessionConnectedEvent → SADD chat:online:{userId} {sessionId} + EXPIRE
  safety-net; SessionDisconnectEvent → SREM (offline khi tập rỗng). KHÔNG phải online-indicator — chỉ gate offline-notify.
...
**PHASE 4 — Thông báo offline (Outbox + Kafka → notification-service)** — *UC-CHAT-05 · §G.8 · rule #4/#5/#7*
```
Đọc lại UC_Chatting.md UC-CHAT-05, §E.3 (Kafka), §G.8 + kafka-patterns.md. Plan mode trước.
- chat-service (PRODUCER — làm ngay): khi gửi tin mà người nhận KHÔNG ở chat:online → set notifyStatus=PENDING +
  notifyTarget (STAFF_ASSIGNED/STAFF_GROUP/CUSTOMER) NGAY TRONG message doc (single-doc outbox, atomic).
  ChatOutboxScheduler @Scheduled(3s): quét message notifyStatus=PENDING → kafkaTemplate.send("chat.message.sent",
  payload) → set SENT. KafkaConsumerConfig + DefaultErrorHandler + DLT chat.message.sent.DLT (backoff 2/4/8s ×3) +
  ChatDeadLetterMonitor (mirror SlotDeadLetterMonitor: log [DLT] + counter tag topic, KHÔNG auto-reprocess).
- notification-service (CONSUMER): NẾU notification-service đã build (Day 13) → thêm @KafkaListener("chat.message.sent",
  containerFactory="manualAckListenerContainerFactory") idempotency theo messageId (Mongo dedupe) → gửi email/FCM
  "Bạn có tin nhắn hỗ trợ mới" → ack. NẾU CHƯA build → BỎ QUA consumer phase này (ghi TODO), tin vẫn an toàn ở
  Kafka/DB, chỉ chậm notify; làm consumer khi tới Day 13.
Test: IT — gửi tin khi recipient offline → message PENDING → scheduler publish chat.message.sent → SENT. mvn verify xanh.
```

**PHASE 5 — Frontend (`@stomp/stompjs`)** — *UC-CHAT-01..04 FE · §G.2/G.3*
```
...
- Khách: nút nổi "Hỗ trợ" ở trang booking/chi tiết sân → mở khung chat (POST /conversations idempotent) →
  list tin (keyset, cuộn lên load thêm) + ô nhập + đính ảnh + typing + tick "đã gửi/đã nhận/đã xem" (3-state).
  SUBSCRIBE /user/queue/messages·delivery·read·typing. ESCAPE nội dung khi render (chống stored-XSS, §G.3).
- STAFF: tab "Hỗ trợ" trong frontend/src/pages/admin/AdminPage.tsx — 2 danh sách: "Của tôi" (inbox riêng) +
  "Hàng đợi chưa gán"; SUBSCRIBE /topic/staff.queue (hàng đợi) + /user/queue/inbox (thread của tôi); nút
  Nhận(claim)/Chuyển(transfer)/Nhả(release)/Đóng(close) + khung chat tái dùng. Badge unread = tổng staffUnread.
- Theme bám alobo.vn (dark-green + gold), toast react-hot-toast, React Query cho REST.
Verify: npm run build xanh; click-test khách↔STAFF gửi/nhận tức thì, 3-state, ảnh, claim/transfer/release, offline reconnect.
```

**PHASE 6 — Test tổng + go-live checklist (§G.10)**
```
...
- Unit: dedupe clientMsgId · validation ≤2000/blank · claim race 409 · authz (USER không thấy thread STAFF khác;
  participant-check). IT (Testcontainers Mongo+Redis+Kafka): send→persist→push · keyset pagination · offline→outbox→
  chat.message.sent · WS CONNECT thiếu/sai token reject · SUBSCRIBE /topic/staff.queue bởi USER → 403 ·
...
- Go-live checklist §G.10: CONNECT-frame auth + per-frame authz bật · rate_limit:chat enforce · MIME+≤5MB ảnh ·
  1 instance + alert log [DLT] + ChatDeadLetterMonitor · CLOUDINARY_* (prod guard) · origin=FRONTEND_URL ·
  reconcile-unread + presence ref-count chạy.
mvn -pl chat-service verify xanh. Tắt mọi service Claude bật ngầm; user tự run để kiểm tra tay.
```

**Định nghĩa Done**: 5 UC chạy e2e qua FE · **per-staff inbox đúng** (STAFF khác KHÔNG thấy thread đã gán · USER
KHÔNG subscribe được `/topic/staff.queue` · SEND spoof bị chặn) · 3-state `sent→delivered→read` hiển thị + typing ·
gửi ảnh **MIME allowlist + ≤5MB** · offline → email/FCM (khi có notification-service) · **keyset pagination** lịch sử ·
**1 conversation mở/khách** (unique sparse `activeCustomerId`) · presence ref-count (multi-device) · `mvn verify` xanh ·
trước pilot pass **checklist §G.10**. chat-service là service Java mới (đã vào root pom) · **1 instance** (simple
broker in-memory — multi-instance cần relay, §D.11/§G.8).
...
> 🖥️ **Toàn bộ frontend đã gộp vào Day 4** — xem mục **"Frontend — toàn bộ app"** cuối Day 4 (scaffold + auth +
> courts/booking grid + matchmaking/real-time + admin/coach + events). Các Day 16–19 để trống; dùng cho việc khác
> nếu cần (vd polish, e2e UI, hoặc kéo backend Week 2–3 lên sớm).

---

### Day 20: event-service + final integration
> (ai-service stub cũ đã được thay bằng ai-service THẬT ở **Day 10.5** — Day 20 chỉ còn event-service + smoke test.)
...
PHẦN B — ai-service: ĐÃ THAY bằng ai-service THẬT ở Day 10.5 (đối soát chứng từ AI, Python).
- KHÔNG làm stub ở đây nữa — xem mục "Day 10.5". (Nếu RAG chatbot GET /api/ai/chat cần sau thì là backlog riêng.)

(Frontend EventsPage đã chuyển sang Day 4 — mục "Frontend — toàn bộ app".)

PHẦN C — end-to-end smoke test toàn bộ luồng (court booking, match host+join, coach enroll,
event ticket, STAFF admin ops). Fix critical bug.
```

**Định nghĩa Done**: mua vé event chạy end-to-end; smoke test toàn hệ thống pass (ai-service thật đã ở Day 10.5).

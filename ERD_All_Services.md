...
erDiagram
    ...
    %% Owns: courts, time slots, slot auto-gen, court reviews, geo
    %% ═══════════════════════════════════════════════════════
    courts {
        UUID        id                          PK
        ...
        decimal     price_per_hour                      "NOT NULL · VND · used to calculate court_price"
        ...
        decimal     rating                              "DECIMAL(3,2) DEFAULT 0.00 · avg of court_reviews"
    ...
    time_slots {
        ...
        UUID        booking_id                          "ref bookings.id · nullable · set when RESERVED via booking"
        timestamp   created_at                          "NOT NULL DEFAULT NOW()"
        timestamp   updated_at                          "NOT NULL DEFAULT NOW()"
    }
    court_reviews {
        UUID        id                          PK
        UUID        court_id                            "ref courts.id · cross-service UUID"
    ...
    %% Owns: court bookings, idempotency guard, cancellation policy
    %% ═══════════════════════════════════════════════════════
    bookings {
        UUID        id                          PK
        UUID        user_id                             "ref users.id · cross-service UUID"
        UUID        slot_id                             "ref time_slots.id · cross-service UUID"
        UUID        court_id                            "ref courts.id · cross-service UUID"
        UUID        cancelled_by                        "ref users.id · nullable · who cancelled"
        decimal     total_price                         "NOT NULL · VND"
        decimal     refund_amount                       "nullable · actual amount refunded per cancellation policy"
        string      status                              "PENDING|CONFIRMED|CANCELLED|COMPLETED"
        string      cancel_reason                       "nullable"
        timestamp   match_start_time                    "NOT NULL snapshot · used for cancellation policy calculation"
    ...
    matches {
        ...
        time        start_time                          "NOT NULL"
        time        end_time                            "NOT NULL"
        int         total_slots                         "NOT NULL · EVEN number: 2|4|6|8|10|12"
    ...
    escrow_transactions {
        ...
        string      reference_payment_id                "ref payments.id · nullable · the bank payment that triggered this"
    ...
    events {
        ...
        int         price_per_ticket                    "NOT NULL · VND"
    ...
    courts              ||--o{    court_reviews           : "receives reviews"
    ...
    users               }o..o{    court_reviews           : "reviews court (cross-DB)"
    ...
    %% courts ↔ services
    courts              }o..o{    bookings                : "booked by user (cross-DB)"
    courts              }o..o{    matches                 : "hosts match (cross-DB)"
    courts              }o..o{    events                  : "hosts event (cross-DB)"

    %% time_slots ↔ services
    time_slots          }o..o{    matches                 : "reserved by match (cross-DB)"
    time_slots          }o..o{    bookings                : "reserved by booking (cross-DB)"
    time_slots          }o..o{    coach_enrollments       : "reserved for coaching (cross-DB)"
    ...
    bookings            }o..o{    court_reviews           : "enables review (cross-DB)"
    ...
    %% escrow ↔ services
    escrow_transactions }o..o{    payments                : "references VNPay payment (cross-DB)"

    %% coach enrollments ↔ services
    coach_enrollments   }o..o{    payments                : "paid via VNPay (cross-DB)"
    coach_enrollments   }o..o{    notification_history    : "triggers notification (cross-DB)"

    %% events ↔ services
    event_tickets       }o..o{    payments                : "paid via VNPay (cross-DB)"
...
| `court-service` | `court_db` | PostgreSQL | 3002 | Courts · Slots · Slot Auto-Gen · Court Reviews · Geo Search |
| `booking-service` | `booking_db` | PostgreSQL | 3003 | Court Bookings · Cancellation Policy · Idempotency Guard |
| `matchmaking-service` | `matchmaking_db` | PostgreSQL | 3004 | Matches · Saga · Outbox · Waiting List · Socket.io |
| `payment-service` | `payment_db` | PostgreSQL | 3006 | VNPay Charge · Real Refund API · Receipt |
...
| System timeout (no payment in 15 min) | Scheduler | N/A (no players) | Host refunded full `court_price` |
...
| 1 booking → 1 court review | `court_reviews.booking_id UNIQUE` constraint |
...
| Slot locked during Host payment (15 min) | Redis `lock:slot:{slotId}:match_create` TTL 15m + `time_slots.match_id` ref |

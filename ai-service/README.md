...
Conversational **booking concierge** for BadmintonHub — FastAPI · LangGraph · Gemini (provider-agnostic) · MCP.
...
- Repo-root `.env` with `JWT_SECRET`, `AI_DB_URL`, `GEMINI_API_KEY`, `GATEWAY_URL`, `EUREKA_URL` (see `../.env.example`).

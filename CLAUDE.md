...
1. **Day 4** — `user-service` auth core (register, email verify, login, refresh token, logout)
   ...
   - ✅ Nền test đã sẵn: sau khi build → gõ `/write-tests user-service auth` (unit + integration `*IT` extends `AbstractIntegrationTest`, `JwtTestTokens`, `mvn verify` xanh). Đây là service ĐẦU TIÊN có test tự động

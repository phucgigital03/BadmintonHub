...
**Cập nhật lần cuối**: 2026-06-08 (Day 4 auth core xong + convention interface/impl + gom toàn bộ frontend vào Day 4)
...
- **`IMPLEMENTATION_GUIDE.md` — Day 4 đồng bộ current state + gom TOÀN BỘ frontend vào Day 4 (docs-only, CHƯA commit)**:
  ...
  - **Mục con mới "🖥️ Frontend — toàn bộ app" trong Day 4**: gom scaffold (Vite/React/TS/Tailwind) + axiosClient + authStore + auth pages + courts/booking grid + matchmaking/realtime + admin/coach + events (trước đây tách Day 16–19 + EventsPage Day 20). ⚠️ Rule degradation: API chưa có → React Query `isLoading/isError` → empty/error state + toast, KHÔNG crash
...
- **Chưa commit**: `CLAUDE.md` (handoff trước) + `IMPLEMENTATION_GUIDE.md` (gom frontend) — docs-only, đợi user quyết commit
- **Frontend chưa scaffold** — chỉ có `frontend/.env`/`.env.example`; user dự định build toàn bộ FE theo mục mới ở Day 4

### 📋 Việc tiếp theo (theo thứ tự ưu tiên)
1. **Build frontend (mục "Frontend — toàn bộ app" ở Day 4)** — scaffold Vite/React/TS/Tailwind + axiosClient + authStore + auth pages (chạy thật với gateway :3000) + các page còn lại (courts/match/admin/coach/events) render empty/error state vì backend chưa có. User muốn build tối nay
2. **Test + verify Day 4** — `/write-tests user-service auth` (unit `AuthServiceTest` + integration `AuthControllerIT` extends `AbstractIntegrationTest`, `JwtTestTokens`, `mvn verify` xanh); rồi smoke end-to-end qua gateway. ĐÂY là service ĐẦU TIÊN có test tự động
3. **Hoàn thiện `.env`** — điền credentials thật: Google OAuth2, SendGrid, Cloudinary, OpenAI, FCM (SendGrid để trống thì email verify chỉ log link ra console — dev vẫn chạy)
4. **Day 5** — `user-service` OAuth2 Google + profile endpoints + `court-service` scaffold (qua `/new-service`)

...
4. **Outbox Pattern in `matchmaking-service`.** Save `OutboxEvent` in the same `@Transactional` as the business record. Never publish Kafka directly from a service method.
5. **Idempotency guard in `booking-service` and `escrow-service`.** Check `processed_events` before handling any Kafka event.

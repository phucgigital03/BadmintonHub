...
4. **Outbox Pattern in `matchmaking-service`, `booking-service`, and `payment-service`.** Save `OutboxEvent` in the same `@Transactional` as the business record. Never publish Kafka directly from a service method. (booking-service uses it for the `booking.slot.held` / `booking.slot.released` slot-hold Saga + the `booking.payment.orphaned` / `booking.refund.required` compensations; payment-service for all `payment.*` events.)

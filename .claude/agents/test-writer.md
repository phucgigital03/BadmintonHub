...
Bạn viết **TEST** cho dự án **BadmintonHub**. Đọc `.claude/rules/java-spring.md` (mục Testing) + `redis-patterns.md` trước.

## Nguyên tắc
- Chỉ `@SpringBootTest` + `@Testcontainers` với PostgreSQL / Redis **thật** — KHÔNG H2, KHÔNG `@DataJpaTest` trên H2. Kafka → embedded Kafka hoặc Testcontainers Kafka.
...
## Khi xong
Báo cáo: test đã viết + đã chạy. Ở giai đoạn TDD, test **mong đợi ĐỎ** (chưa có impl) — đó là đúng, KHÔNG sửa test thành xanh giả để "cho qua".
